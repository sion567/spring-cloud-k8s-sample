...
public class OrderController {
     ...
     */
    @GetMapping("/me")
    public Result<PageResult<OrderResponseDTO>> getMyOrders(
            @RequestHeader(name = "X-User-Id", required = true) Long userId,
            @Valid PageRequest request) { // 包装分页参数：page, size

        return Result.success(orderService.getUserOrders(userId, request));
    }
